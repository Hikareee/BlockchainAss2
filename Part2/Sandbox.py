...
def hash_message(value):
    # Convert integer to string and encode to bytes
    value_str = str(value).encode()
    
    # Generate MD5 hash in hexadecimal format
    hex_hash = hashlib.md5(value_str).hexdigest()
...
def run_query():
    ...
    signature_bit_length = po_n.bit_length()
    response = (quantity << signature_bit_length) | final_signature

    if response >= po_n:
        output.insert(END, "❌ Response too large to encrypt. Adjust key sizes or packing scheme.\n")
        return
